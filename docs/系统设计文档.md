...
- 本地 OCR：图片拖拽/上传后生成项目相对路径，由 `ocr_image` 工具调用 PaddleOCR PP-OCRv6 small 识别。
...
2. 前端 `useChat` 过滤 `png`、`jpg`、`jpeg`、`bmp`、`webp`、`tif`、`tiff` 图片。
3. 浏览器 `File` 上传会转为 base64；系统拖拽路径会保留为 `source_path`。
4. `save_chat_image_attachment` 在后端校验项目根、扩展名、大小和文件名，把图片保存到 `.nano-agent/uploads/images/`。
5. 后端返回项目相对路径，前端把路径插入输入框，提示模型需要识别时调用 `ocr_image`。
6. 模型输出 `ocr_image` 工具调用后，仍进入统一 Agent 工具审批与执行链路。
7. `ocr_image` 调用本机 PaddleOCR CLI，使用 PP-OCRv6 small 检测与识别模型，结果作为工具执行结果回写对话。
...
- 图片附件保存到项目内 `.nano-agent/uploads/images/`，该目录不进入项目文件树和 git 跟踪；OCR 仍只能读取项目内相对路径。
- 高风险工具如 `write_file`、`execute_command` 必须经过用户审批。
- 命令执行默认在项目目录下运行，并由前端 Skills 开关控制 Bash Tool 是否可执行。
- RAG 单文件文本有大小限制，避免一次性索引超大内容。
- OCR 附件保存限制为 25MB；实际识别限制为 8MB，并通过检测最长边、batch、线程数和超时控制本地资源占用。
