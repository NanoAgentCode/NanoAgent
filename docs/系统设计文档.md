...
- 运行时可观测：LLM、MCP、Ops、工具调用等行为写入独立观测库并在设置中展示。
...
- Tauri command 是前后端边界，所有数据库、文件系统、命令执行、模型请求、MCP、OCR 和 Ops 操作都经由 command。
...
| 主业务存储 | `src-tauri/src/db.rs` | items、model_configs、mcp_servers、conversations、messages、RAG、memories、ops_servers |
...
5. 前端读取启用记忆、项目文件树、RAG 召回、启用 Skills 和已连接 MCP 工具。项目文件树同时作为前端消息渲染的轻量文件索引来源。
...
| 数据库 | 职责 | 主要表 |
| --- | --- | --- |
| `nano-agent.sqlite3` | 主业务数据 | `items`、`model_configs`、`mcp_servers`、`conversations`、`messages`、`rag_*`、`memories`、`ops_servers` |
