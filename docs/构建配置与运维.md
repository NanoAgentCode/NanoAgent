...
`nano` 与桌面端复用同一个 Rust LLM 实现、模型配置数据库和项目索引存储。CLI 对话历史只保存在当前进程内，退出后不会写入桌面端会话列表。
...
CLI 默认读取桌面端 `com.nanoagent.desktop` 应用数据目录中的模型配置，因此首次使用前需要先在桌面端配置至少一个聊天模型。项目模式会更新共享数据库中的代码和文档索引；临时模式不会绑定或扫描项目。
