...
1. 用户在项目区点击代码索引按钮。
2. 前端调用 `index_project_code(projectPath)`。
3. Rust 后端扫描项目、抽取实体、生成关系和代码片段。
4. 数据写入 `code_index_runs`、`code_entities`、`code_relations`、`code_chunks`、`code_chunks_fts` 和可选的 `code_embeddings`。
5. 聊天发送前，如果当前问题像代码问题，前端调用 `search_code_index` 检索当前项目代码索引。
6. 命中的路径、行号、实体和片段注入 system message，模型回答时优先引用这些上下文。

代码索引是项目级能力，不绑定单个会话；普通 RAG 仍然是会话级上传文件能力。
...
- 代码索引首版使用轻量规则解析，适合定位和问答增强；复杂动态调用仍需读取精确文件或后续接入更完整 AST 引擎。
