...
`nano` 默认以当前目录作为项目并在启动时更新代码/文档索引，项目会话自动保存。使用 `nano --continue` 恢复最近会话，`nano --sessions` 查看会话，`nano --temp` 启动不绑定项目、不保存历史的普通临时对话。详见[构建、配置与运维](docs/构建配置与运维.md#2-nano-终端客户端)。
